...
class GenericNetlinkSocket:
    ATTRIBUTES: Dict[int, attributes.AttributeType] = {}
...
class GenericNetlinkController(GenericNetlinkSocket):
    ...
    ATTRIBUTES = {
        ...
        CTRL_ATTR_MCAST_GROUPS: attributes.array(
            attributes.nested(ATTRIBUTES_MCAST_GRP), base=1
        ),
        CTRL_ATTR_POLICY: attributes.dict(
            attributes.dict(attributes.nested(attributes.ATTRIBUTES_POLICY_TYPE))
        ),
    ...
    async def get_family_by_name(self, name):
        attrs = {CTRL_ATTR_FAMILY_NAME: name}
        messages = await self.request(CTRL_CMD_GETFAMILY, attrs)
        return Family(messages[0].attributes)
