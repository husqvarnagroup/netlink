...
class AttributeType:
    ...
    def encode(self, value):
        if self.type == AttributeType.U8:
            return struct.pack("B", value)
        elif self.type == AttributeType.U16:
            return struct.pack("H", value)
        elif self.type == AttributeType.U32:
            return struct.pack("I", value)
        elif self.type == AttributeType.U64:
            return struct.pack("Q", value)

        elif self.type == AttributeType.S8:
            return struct.pack("b", value)
        elif self.type == AttributeType.S16:
            return struct.pack("h", value)
        elif self.type == AttributeType.S32:
            return struct.pack("i", value)
        elif self.type == AttributeType.S64:
            return struct.pack("q", value)
        ...
        elif self.type == AttributeType.NESTED:
            return encode(value, self.map)
        elif self.type == AttributeType.ARRAY:
            attrs = {
                i + self.base: self.etype.encode(item) for i, item in enumerate(value)
            }
        ...
        elif self.type == AttributeType.FLAG:
            return b""
        elif self.type == AttributeType.PADDING:
            return b""
    ...
    def decode(self, data):
        if self.type == AttributeType.U8:
            ...
            return decode(data, self.map)
        elif self.type == AttributeType.ARRAY:
            return [self.etype.decode(val) for val in decode_raw(data).values()]
        elif self.type == AttributeType.DICT:
            return {
                key: self.etype.decode(val) for key, val in decode_raw(data).items()
            }
...
def u8():
    return AttributeType(AttributeType.U8)


def u16():
    return AttributeType(AttributeType.U16)


def u32():
    return AttributeType(AttributeType.U32)


def u64():
    return AttributeType(AttributeType.U64)


def s8():
    return AttributeType(AttributeType.S8)


def s16():
    return AttributeType(AttributeType.S16)


def s32():
    return AttributeType(AttributeType.S32)


def s64():
    return AttributeType(AttributeType.S64)
...
def nested(map):
    return AttributeType(AttributeType.NESTED, map=map)


def array(etype, *, base=0):
    return AttributeType(AttributeType.ARRAY, etype=etype, base=base)


def dict(etype):
    return AttributeType(AttributeType.DICT, etype=etype)


def flag():
    return AttributeType(AttributeType.FLAG)


def padding():
    return AttributeType(AttributeType.PADDING)
